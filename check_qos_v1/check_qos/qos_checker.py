...
def rule_durable_keep_last_depth(_xml, q):
    ...
    return None
# ────────── 규칙 7 : durability + Keep_All + max_samples=INF──────────
def rule_keepall_durable_unlimited(_xml, q):
    ...
    return None
# ────────── 규칙 8 : durability + WriterDataLifecycle──────────
...
def rule_deadline_with_best_effort(xml, q):
    ...
    return None
# ────────── 규칙 11 : ownership + deadline + reliability ──────────
...
def rule_exclusive_best_effort_deadline(xml, q):
    ...
    return None
# ────────── 규칙 12 : writerdatalifecycle + reliability ──────────
...
# ────────── 규칙 14 : publish_rate + lifespan + history ──────────
...
def rule_buffer_capacity_vs_lifespan(xml, q):
    ...
    return None
# ────────── 규칙 17 : destination order + history depth ──────────
...
# ────────── 규칙 18 : destination order(pub,sub)──────────
...
def rule_durable_keep_last_depth(xml, q):
    dur_kind = q.get("durability", "").strip().upper()
    hist_kind = q.get("history", "").strip().upper()
    ...
    if depth < required_depth:
        return (f"Invalid QoS: DURABILITY.kind = {dur_kind}, KEEP_LAST({depth}) is too shallow.\n"
                f"Required depth ≥ ⌈RTT / PP⌉ + 2 = ⌈{rtt_sec:.3f}s / {pp_sec:.3f}s⌉ + 2 = {required_depth}.\n"
...
# ────────── 규칙 ──────────
RULES = [
    rule_durability_needs_rel,
    rule_durability_exclusive,
    rule_dstorder_requires_rel_dur, 
    rule_deadline_vs_durability,
    rule_keep_last_sample_budget,
    rule_durable_keep_last_depth,
    rule_keepall_durable_unlimited,
    rule_autodispose_vs_durability,
    rule_lease_vs_deadline,
    rule_deadline_with_best_effort,
    rule_exclusive_best_effort_deadline,
    rule_autodispose_with_best_effort,
    rule_lifespan_vs_deadline,
    rule_history_vs_lifespan,
    rule_exclusive_with_deadline,
    rule_buffer_capacity_vs_lifespan,
    rule_dest_order_vs_depth,
    rule_keep_last_depth_positive,
    rule_history_vs_max_per_instance,
    rule_best_effort_exclusive,
    rule_announce_vs_lease,
    rule_autoenable_vs_volatile_reader,
    rule_max_samples_vs_per_instance,
    rule_destorder_keepall_mpi,
    rule_rdlife_autopurge_vs_durability,
    rule_liveliness_manual_partition,
    rule_autodispose_with_exclusive,
    rule_lifespan_too_short_for_durability,
    rule_exclusive_lease_infinite,
    rule_nowriter_delay_vs_infinite_lease,
    rule_reliable_keep_last_depth_too_small,
    rule_keepall_max_samples_per_instance,
    rule_lifespan_too_short_for_reliability,
    rule_best_effort_with_manual_liveliness,
    rule_deadline_too_short_for_exclusive,
    rule_lease_too_short_for_exclusive,
    rule_keepall_durable_instance_budget,
    rule_durable_keep_last_depth,
    rule_exclusive_deadline_infinite,
    rule_lifespan_exceeds_per_instance,
...
def main() -> None:
    ...
    for side, (xml, prof) in (("PUB", (pub_xml, pub_q)),
                              ("SUB", (sub_xml, sub_q))):
        for rule in RULES:                  # RULES : (xml, prof) 형식만!
            msg = rule(xml, prof)
            if msg:
                warnings.append(
                    f"{color('[WARN]', RED)} "
                    f"{color(f'[{side}]', BLUE)} {msg}"
                )

    # ── 2) 교차-규칙(Writer + Reader 모두 필요) 호출 ───────────
# ── 2) 교차-규칙 호출 ─────────────────────────────
    for cross_rule in (
            rule_dest_order_compat,
            rule_ownership_compat,
            rule_reliability_compat,
            rule_durability_compat,
            rule_deadline_period_compat,        # xml 2개
            rule_liveliness_compat,             # xml 2 + dict 2
            rule_nowriter_autodispose_cross,    # dict 2
            rule_partition_userdata_key,        # dict 2
            rule_partition_overlap,             # ★ xml 2개
            rule_durable_partition_miss,        # xml 2 + dict 2
            rule_deadline_partition_reset,      # ★ xml 2개
            rule_liveliness_incompatibility
        ):
        ...
        if cross_rule in {
            ...
            msg = cross_rule(pub_xml, sub_xml)

        elif cross_rule is rule_liveliness_compat:
            msg = cross_rule(pub_xml, sub_xml, pub_q, sub_q)
            
        elif cross_rule is rule_liveliness_incompatibility: 
            msg = cross_rule(pub_xml, sub_xml, pub_q, sub_q)

        elif cross_rule is rule_durable_partition_miss:
            msg = cross_rule(pub_xml, sub_xml, pub_q, sub_q)

        else:                                   # dest_order / ownership 등
            msg = cross_rule(pub_q, sub_q)

        if msg:
            warnings.append(f"{color('[WARN]', RED)} {msg}")
